...
- **M5b — Edges and narrow supports.** Done (2026-09-11), in-game verified: feet hanging over an edge gather onto the support (joint radial search from both ankles, cheapest uncrossed pair with room between the boots, hold while still, re-plant only when a turn crosses the legs), feet kept out of walls by a boot-sized box, body centred over the feet through the movement hook, leg balance across heights, a minimum stance sideways and a boot length lengthwise, shallow water looked through to the bed, no gathering while the gate is closed (a fall is flagged as jumping), level support preferred over lower ground within Edge drop unless the character is on an incline (ground half a leg ahead and behind differ by more than a tread), and the emote-only body baseline (`ProbeBase`) never used while walking: on a rail the ground under the body is the deck. Open: on a rail the feet must swap ends once per half turn; unavoidable without crossed legs.
